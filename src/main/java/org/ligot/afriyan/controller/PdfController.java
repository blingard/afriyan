...
public class PdfController {
    ...
    private final IRendezVous iRendezVous;

    public PdfController(ISouscription service, IRendezVous iRendezVous) {
        this.service = service;
        this.iRendezVous = iRendezVous;
    }

    @GetMapping("/{id}")
    public void generatePdf(@PathVariable("id") Long id, HttpServletResponse response) throws Exception {
/*        RendezVousDTO rendezVousDTO = iRendezVous.findById(id);
        response.setContentType("application/pdf");
        response.setHeader("Content-Disposition", "attachment; filename=abonnement"+rendezVousDTO.getServiceEntity().getCentrePartenaire().getLibelle()+".pdf");

        OutputStream out = response.getOutputStream();

        Document document = new Document(PageSize.A10);
        PdfWriter.getInstance(document, out);

        document.open();
        header(document);
        body(document, rendezVousDTO, out);
        document.close();*/

        RendezVousDTO rendezVousDTO = iRendezVous.findById(id);
        response.setContentType("application/pdf");
        response.setHeader("Content-Disposition", "attachment; filename=abonnement" + rendezVousDTO.getId().toString()+ ".pdf");

        OutputStream out = response.getOutputStream();
        Document document = new Document(PageSize.A6); // Modifié de A10 à A4
        ...
        addWatermark(writer); // Spécifiez le chemin de votre image de filigrane
    ...
    private void header(Document document) throws DocumentException, IOException {
        ...
        Image leftLogo = Image.getInstance(young); // Spécifiez le chemin de l'image
        ...
        Image rightLogo = Image.getInstance(unfpa); // Spécifiez le chemin de l'image
    ...
    private void body(Document document, RendezVousDTO rendezVousDTO) throws DocumentException, IOException {
        ...
        CustomParagraph(table, "Service: ", rendezVousDTO.getServiceEntity().getLibelle());
        leaveEmptyLine(paragraph, 1);
        CustomParagraph(table, "Centre Partenaire: ", rendezVousDTO.getServiceEntity().getCentrePartenaire().getLibelle());
        leaveEmptyLine(paragraph, 1);
        String pattern = "dd/MM/yyyy hh-mm";
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
        CustomParagraph(table, "Date: ", simpleDateFormat.format(rendezVousDTO.getDateRdv()));
        leaveEmptyLine(paragraph, 1);
    ...
    private void addWatermark(PdfWriter writer) throws DocumentException, IOException {
        Image watermark = Image.getInstance(young);
        //watermark.setOpacity(0.5f); // Ajuster l'opacité du filigrane
        watermark.scaleToFit(400, 300); // Ajuster la taille selon vos besoins

        // Positionner le filigrane au centre de la page
        PdfContentByte canvas = writer.getDirectContentUnder();
        float x = (writer.getPageSize().getWidth() - watermark.getScaledWidth()) / 2;
        float y = (writer.getPageSize().getHeight() - watermark.getScaledHeight()) / 2;
...
/*
    ...
    private void body(Document document, RendezVousDTO rendezVousDTO, OutputStream out) throws DocumentException, IOException {
        Paragraph paragraph = new Paragraph();
        leaveEmptyLine(paragraph, 2);
        document.add(paragraph);
