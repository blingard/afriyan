package org.ligot.afriyan.Dto;

import org.ligot.afriyan.entities.TypeDonne;

import java.util.Date;

public class ArticlesSmart {
    private Long id;
    private String title;
    private String author;
    private String resumer;
    private boolean status;
    private String phote;
    private TypeDonne typeDonne;

    private CategoriesDTO categories;

    private int lue;

    public ArticlesSmart() {
    }

    public ArticlesSmart(Long id, String title, String author, String resumer, boolean status, String phote, TypeDonne typeDonne, CategoriesDTO categories, int lue) {
        this.id = id;
        this.title = title;
        this.author = author;
        this.resumer = resumer;
        this.status = status;
        this.phote = phote;
        this.typeDonne = typeDonne;
        this.categories = categories;
        this.lue = lue;
    }

    public ArticlesSmart(Long id) {
        this.id = id;
    }

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getTitle() {
        return title;
    }

    public void setTitle(String title) {
        this.title = title;
    }

    public String getAuthor() {
        return author;
    }

    public void setAuthor(String author) {
        this.author = author;
    }

    public String getResumer() {
        return resumer;
    }

    public void setResumer(String resumer) {
        this.resumer = resumer;
    }

    public boolean isStatus() {
        return status;
    }

    public void setStatus(boolean status) {
        this.status = status;
    }

    public TypeDonne getTypeDonne() {
        return typeDonne;
    }

    public void setTypeDonne(TypeDonne typeDonne) {
        this.typeDonne = typeDonne;
    }

    public String getPhote() {
        return phote;
    }

    public void setPhote(String phote) {
        this.phote = phote;
    }

    public CategoriesDTO getCategories() {
        return categories;
    }

    public void setCategories(CategoriesDTO categories) {
        this.categories = categories;
    }

    public int getLue() {
        return lue;
    }

    public void setLue(int lue) {
        this.lue = lue;
    }
}
